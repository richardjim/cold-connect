...
public class MarketplaceController extends BaseController {
    ...
    public record PlaceOrderRequest(
            ...
            @Schema(example = "DELIVERY", description = "DELIVERY · COLLECTION")
            @NotBlank String fulfilmentType,
            String destAddress,
            @Schema(example = "PREPAID", description = "PREPAID · PAY_ON_DELIVERY · CREDIT")
            String paymentPreference
    ) {}

    @Operation(summary = "Browse live lots — filter by commodity")
    @GetMapping("/lots")
    public ResponseEntity<List<MarketLot>> browseLots(
            @RequestParam(required = false) String commodity) {
        return ResponseEntity.ok(marketplaceService.browseLots(commodity));
    }

    @Operation(summary = "Get lot detail")
    ...
    @Operation(summary = "Get cold-chain for a lot")
    ...
    @PostMapping("/lots")
    public ResponseEntity<MarketLot> listLot(
    ...
    @PostMapping("/orders")
    public ResponseEntity<MarketOrder> placeOrder(
            @AuthenticationPrincipal UserDetails userDetails,
            @Valid @RequestBody PlaceOrderRequest req) {
        Long buyerId = resolveUser(userDetails).getId();
        return ResponseEntity.ok(marketplaceService.placeOrder(
                buyerId, req.items(), req.fulfilmentType(),
                req.destAddress(), req.paymentPreference()));
    }
    ...
    public ResponseEntity<List<MarketOrder>> getMyOrders(
        ...
        return ResponseEntity.ok(
                marketplaceService.getBuyerOrders(resolveUser(userDetails).getId()));
    }
